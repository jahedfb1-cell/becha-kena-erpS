...
const Orders = () => {
  ...
  // {value, label, sublabel, search} contract the quotation builder and
  // PriceListModal already use for their product pickers, so switching a
  // line item's product behaves the same familiar, searchable way here too
  // instead of the old plain <select> + separate filter box.
        ...
        blocks: sec.blocks.map(block => {
          ...
          const updatedSizes = block.sizes.map(size => {
            ...
            if (isPcs) {
              ...
              const isPvc = _blockUnitO.includes('pvc');
              if (isPvc) {
          ...
          if (field === 'unit_price' || field === 'product_id') {
            ...
            updatedBlock.sizes = updatedBlock.sizes.map(size => {
              ...
              if (isPcs) {
                ...
                const isPvc = _updUnitO.includes('pvc');
                if (isPvc) {
  ...
  const handleEditClick = async (o) => {
    ...
    try {
      ...
      (fullQ.items || []).forEach(item => {
        ...
        const isPvc = _prodUnitLcO.includes('pvc');
                  ...
                  <div key={sec.id} className="form-card-section mobile-simple-section" style={{ marginBottom: '24px', border: '1px solid #cbd5e1', borderRadius: '8px', padding: '6px' }}>
                    ...
                    {sec.blocks.length === 0 ? (
                      ...
                      sec.blocks.map((block) => {
                        ...
                        const isPvcBlock = _blkUnitO.includes('pvc');
                          
                        const columnTitles = isPvcBlock ? ORDER_COLUMNS_WITH_PVC : ORDER_COLUMNS;

                        // Reuses the same SearchableSelect combobox as the quotation
                        // builder's "Change Product" and the Select Customer field,
                        // replacing the old plain <select> + separate filter box with
                        // one type-to-search input styled and behaving the same way
                        ...
                        const changeProductUI = productChangeBlockId === block.id ? (
                          <div className="form-group" style={{ margin: 0, minWidth: '260px' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'flex-start', gap: '6px' }}>
                              ...
                              <button
                                type="button"
                                onClick={() => { setProductChangeBlockId(null); setProductChangeQuery(''); }}
                          ...
                          <div key={block.id} style={{ marginBottom: '16px', background: isOptionGroup ? (isSelected ? '#faf5ff' : '#f8fafc') : '#ffffff', border: isOptionGroup ? (isSelected ? '1px solid #c084fc' : '1px dashed #cbd5e1') : '1px solid #e2e8f0', borderRadius: '8px', padding: '12px' }}>
                            ...
                            <div style={{ overflowX: 'auto' }}>
                              <table className="data-table item-builder-table" style={{ width: '100%', margin: 0 }}>
                                {/* Column widths live here now: the header row moved onto each
                                ...
                                <tbody>
                                      ...
                                      <div className="mobile-size-card">
                                        <div className="mobile-size-header-bar">
                                          {/* A per-piece item has no width or height, so the card
                                              drops those two boxes rather than showing them dead. */}
                                          {!isPcsBlock && (
                                            <>
                                  ...
                                  {/* This is the "Description of Goods" text that prints under
                                      ...
                                      print side already renders it through renderRichText(), so
                                      editing it as rich text here isn't a new capability, just
                                      finally a way to produce the markup that side expects. */}
                                  <tr style={{ background: '#f8fafc' }}>
                                    <td colSpan={ORDER_COLUMNS.length - 1} style={{ padding: '8px 14px' }}>
                                      <RichTextEditor
                                        value={block.notes || ''}
                  ...
                  {vatEnabled && (
                    <>
                      <input
                        ...
                        style={{ marginTop: '6px' }}
                      />
                      {/* Which way the agreed price was struck. Exclusive adds
                      ...
                      <div style={{ fontSize: '11px', color: 'var(--text-muted, #64748b)', marginTop: '4px' }}>
                        {vatInclusive
                          ? 'Customer pays the agreed price; the VAT challan splits it.'
                          : 'VAT is charged above the agreed price.'}
                      </div>
                    </>
                  )}
                </div>
